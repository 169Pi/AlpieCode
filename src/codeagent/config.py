"""
Per-user config for codeagent.

Resolution order (highest priority first):
  1. Environment variables: CODEAGENT_BASE_URL, CODEAGENT_MODEL, CODEAGENT_API_KEY
  2. ~/.codeagent/config.json (written by `codeagent init`)
...
def load_config() -> Config:
    ...
    # env vars win over the saved file
    if os.environ.get("CODEAGENT_BASE_URL"):
        data["base_url"] = os.environ["CODEAGENT_BASE_URL"]
    if os.environ.get("CODEAGENT_MODEL"):
        data["model"] = os.environ["CODEAGENT_MODEL"]
    if os.environ.get("CODEAGENT_API_KEY"):
        data["api_key"] = os.environ["CODEAGENT_API_KEY"]
    return Config(**data)
...
def interactive_init() -> Config:
    current = load_config()
    print("codeagent setup — press enter to keep the current/default value.\n")
